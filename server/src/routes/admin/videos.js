...
function saveCover(dataUrl) {
  try {
    ...
    fs.writeFileSync(path.join(config.uploadsDir, 'covers', name), Buffer.from(m[2], 'base64'));
    return `covers/${name}`;
  } catch { return null; }
}
...
router.get('/', async (req, res, next) => {
  try {
    ...
    let q = db('videos');
    if (req.query.keyword) q = q.where('name', 'like', `%${req.query.keyword}%`);
...
router.post('/upload', requireRoles('super_admin', 'operator'), upload.single('file'), async (req, res, next) => {
  try {
...
router.delete('/:id', requireRoles('super_admin', 'operator'), async (req, res, next) => {
  try {
    ...
    try {
      fs.unlinkSync(path.join(config.uploadsDir, v.file_path));
      if (v.cover) fs.unlinkSync(path.join(config.uploadsDir, v.cover));
    } catch { /* 文件不存在则忽略 */ }
...
// ===== 批量删除 =====
router.post('/batch-delete', requireRoles('super_admin', 'operator'), async (req, res, next) => {
  try {
...
router.put('/:id/audit', requireRoles('super_admin', 'operator', 'auditor'), async (req, res, next) => {
  try {
    const v = await db('videos').where({ id: req.params.id }).first();
    if (!v) return res.status(404).json(fail('素材不存在'));
...
router.get('/selectable/all', async (req, res, next) => {
  try {
    const rows = await db('videos').where({ status: 'approved' }).orderBy('id', 'desc').limit(500);
